...
def enviar_pose(pose, iman_estado, modo):
    # Formato: P:g1,g2,g3,g4,g5,g6,iman,modo
    comando = f"P:{pose[0]},{pose[1]},{pose[2]},{pose[3]},{pose[4]},{pose[5]},{iman_estado},{modo}"
    enviar_mqtt(comando)
...
def ejecutar_trayectoria():
    ...
    for i, pose in enumerate(trayectoria):
        # Lógica de Iman (Igual a la anterior)
        # i=1 (Pick), i=2 (Home con pieza)
        estado_iman = 1 if (1 <= i <= 5) else 0

        # Lógica de Modos de Motor (NUEVA)
        # i=0: Home inicial (usamos modo 4 por seguridad)
        # i=1: Ir a Pick (Modo 1: Pick Ida)
        # i=2: Volver a Home (Modo 2: Pick Vuelta)
        # i=3: Ir a Place (Modo 3: Place Ida)
        # i=4: Volver a Home (Modo 4: Place Vuelta)
        ...
        # Enviamos los 8 parámetros al ESP32
        enviar_pose(pose, estado_iman, modo)
...
def actualizar_interfaz_y_robot(pose):
    ...
    enviar_pose(pose, iman_estado=0, modo=1)  # Envía el comando P:g1,g2... al ESP32
